...
| In / Out of scope | In: a single profile-update service used by both the M31/M33 analysis path and the content-attempt path; a persisted `ProfileObservation` idempotency record keyed by non-null `(player_id, game_id, position_id, skill_id)`; `SkillState` observation applied from player-attributed analysis findings; rating-band priors applied on first observation of a concept; `ReviewSchedule` created when a lesson is generated and updated when it is attempted, with an explicit `skill_id`; `retired_at` and `retirement_reason` metadata on `SkillState` and `ReviewSchedule`; an Alembic revision adding these columns and the observation table with non-retired defaults for existing rows; a live taxonomy-migration hook so a renamed or retired `skill_id` remaps or retires the rows this milestone starts writing. Out: the reports that read this state (M36); adaptive selection (M38). |
| Depends on | M32, M33 |
| Target release | `unversioned — learning loop closed` |
| Deliverables | Profile-update service replacing the duplicated logic in `content/tracking.py:7-40`; Alembic schema upgrade for `ProfileObservation`, `ReviewSchedule.skill_id`, and retained retirement metadata; prior selection from `PlayerProfile.rating` via `learning/profiling/priors.py`; schedule writer storing the diagnosis `skill_id`; taxonomy-migration hook invoking `learning/diagnosis/taxonomy/migration.py` over live `SkillState.concept_code` and `ReviewSchedule.skill_id` rows at startup, marking unmappable rows retired with a reason instead of deleting them. When a renamed `SkillState` conflicts with an existing target row, preserve the target prior, add the old row's evidence above its own prior to the target posterior, retain the later `last_updated`, then retire the old row. `tests/integration/test_schema_migrations.py`, `tests/integration/test_profile_updates_from_analysis.py`, `tests/property/test_mastery_monotonicity.py`, and `tests/integration/test_taxonomy_migration_live.py`. |
| Acceptance | Two games containing the same diagnosis lower that concept's expected mastery monotonically and narrow its uncertainty. A player rated 1200 and a player rated 1900 start a new concept at different priors, neither at `(1.0, 1.0)`. Every generated lesson has a `ReviewSchedule` row with a due date and its diagnosed `skill_id`. Re-analysing the same owned game does not change mastery a second time; an ownerless or game-less finding is rejected before an observation is recorded. A `skill_id` rename applied through the migration table remaps live `SkillState.concept_code` and `ReviewSchedule.skill_id` rows; a target-key collision merges Bayesian evidence deterministically. An unmappable code is retained as retired with a recorded reason, never silently dropped. |
...
| Design reevaluation | Inspect `content/tracking.py:7-40`, `learning/profiling/priors.py:3-20`, `api/learning.py:199-230`, `learning/diagnosis/taxonomy/migration.py:11-51`, the `SkillState` and `ReviewSchedule` schemas, the Alembic chain, and M31–M33's merged owner-attributed paths; confirm G5, G14, G15, and G31 still hold, that analysis findings carry a real player and game rather than synthetic or nullable identities, and that the existing `ReviewSchedule.item_id` cannot represent the taxonomy `skill_id` migration contract without an explicit field and retirement metadata. Dependent milestones requiring review: M36, M37, M38, M42, and transitively M43. |
| Risks & rollback | Risk: double-counting on re-analysis — addressed by the persisted non-null idempotency key, which must be asserted in tests, not assumed. Risk: unifying two update paths changes famous-game attempt behaviour — cover the existing path with a characterisation test before refactoring. Risk: a migration hook that runs at startup can corrupt live rows on a bad table — require a no-op default, a deterministic target-key collision merge, retained retired rows with their reason, and idempotency of the migration itself. Rollback: profile updates are additive and the idempotency key makes recomputation safe. |
| Est. PRs | 6 |
...
| In / Out of scope | In: `recurring_habits` computed by `learning/profiling/habits.py` over the player's persisted diagnoses; the openings report derived from the player's own owner-attributed game corpus with per-family result and error rates; the weekly report as a typed object (games played, active concepts observed, active-mastery deltas, top recurring diagnosis). Retired profile state remains historical data but does not drive current mastery reporting. Out: coach-facing aggregation changes; context-conditioned mastery surfacing. |
| Depends on | M34 |
| Target release | `unversioned — learning loop closed` |
...
| Acceptance | A player with three games sharing one diagnosis has that pattern in `/patterns` with an occurrence count and evidence references. `/openings` reflects that player's actual opening families. `/reports/weekly` returns a typed object whose current mastery fields exclude retired skill rows. No handler in `src/scan64/api/reports.py` returns a hardcoded literal collection or string. |
| Verification | `uv run pytest tests/integration/test_reports_from_real_data.py` — passes; `scripts/check.sh` — exits 0. |
| Design reevaluation | Inspect `api/reports.py:125-129` and the weekly handler, `learning/profiling/habits.py:90-156`, M31's owner-based evidence query, and M34's active/retired profile writes; confirm G10, G11, G12, and G13 still hold. Dependent milestones requiring review: `none`. |
...
| Acceptance | Daily Training presents an interactive board, not a FEN string. Submitting the accepted move is confirmed and recorded; submitting a wrong move consumes an attempt and reveals the next hint. A served training session has a `StudySession` row and every attempt links to it. An attempt changes the relevant active `SkillState`; an attempt for a retired code is recorded with its profile update explicitly skipped and never recreates the code. Completing an Opening Explorer mission records an attempt server-side rather than being discarded on navigation. |
...
| Design reevaluation | Inspect `DailyTrainingScreen.tsx:75-90`, `CriticalMomentReview.tsx:75-129`, `OpeningExplorerScreen.tsx:13-126`, `api/learning.py:250-257`, `content/models.py:39-68`, the merged M34 profile-update service, and the working play-board configuration in `PlayScreen.tsx`; confirm G17, G18, G19, and G23 still hold and that attempts retain learner work without reactivating a retired taxonomy skill. Dependent milestones requiring review: M38, M43. |
...
| In / Out of scope | In: `/v1/learning/session` loads active `SkillState`, due non-retired `ReviewSchedule` rows, recent attempt history, and habit output to compute real priority factors; session fatigue derived from recent attempt volume and accuracy; a documented exploration floor. Out: new exercise types; transfer selection (M43). |
| Depends on | M37 |
| Target release | `unversioned — adaptive and operable` |
| Deliverables | Priority-factor computation replacing the hardcoded block at `api/learning.py:213-230`; documented weighting naming each factor's source; retired-state filtering at the session boundary; `tests/integration/test_adaptive_session.py`. |
...
| Design reevaluation | Inspect `api/learning.py:199-257` and M34/M37's merged active/retired writes; confirm G16 still holds and that `SkillState` and `ReviewSchedule` now carry the active data this milestone depends on. Dependent milestones requiring review: M43. |
...
| In / Out of scope | In: `Evidence`, `Position`, `EngineAnalysis`, `ProfileObservation`, `TransferPosition`, `TransferMeasurement`, and `StudySession` added to the archive schema, the import path, and the deletion path with audit coverage; an enumeration-based completeness test that fails when a player-scoped table is absent from the lifecycle. Out: hosted-mode retention policy (§2 `> GAP:`). |
| Depends on | M31, M34 |
| Target release | `unversioned — adaptive and operable` |
| Deliverables | Extended archive model and handlers in `api/data_lifecycle.py` preserving `Game.owner_player_id` plus `Evidence`, `Position`, `EngineAnalysis`, `ProfileObservation`, `TransferPosition`, `TransferMeasurement`, and `StudySession`; `tests/integration/test_data_lifecycle_completeness.py` enumerating SQLModel tables; deletion audit updated to record the added tables. |
...
| Design reevaluation | Inspect `api/data_lifecycle.py:23-46,118-130,353-365`, the `DeletionAudit` writer at `:367-378`, M31's merged owner and evidence writes, and M34's `ProfileObservation` schema; confirm G28 still holds and that the set of player-scoped tables and owner-bearing fields has not changed. Dependent milestones requiring review: `none`. |
...
| Design reevaluation | Inspect `learning/evaluation/transfer_measurement.py:41-162`, `learning/exercises/transfer.py:13-141`, `learning/verification/verifier.py:26-40`, and M37/M38's merged attempt and active-session paths; confirm G29 and G30 still hold. Dependent milestones requiring review: `none`. |
