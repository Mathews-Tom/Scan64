...
> DECISION (H-002): Imported games have one durable owner. M31 adds nullable `Game.owner_player_id`, migrates existing games as ownerless, and requires `player_id` for new imports. The analysis job, report queries, lesson opportunities, and evidence derive attribution from this field; ownerless legacy games remain read-only historical records, are excluded from player-owned read paths, and are rejected before analysis. `PlaySession` remains an interactive-play record, not an imported-game ownership surrogate.
> DECISION (H-003): M31 bootstraps Alembic and supplies an upgrade path for existing SQLite databases before adding owner and lesson-opportunity columns. A populated legacy SQLite fixture must upgrade with null backfill before any M31 query runs. System design §17.3 also calls for PostgreSQL migration CI; the current SQLite-only repository has no PostgreSQL service or driver, so M31 records that unimplemented cross-database verification as a visible future-planning gap rather than claiming it.
...
> ASSUMPTION: The `Evidence` idempotency key for repeat analysis (M34) is `(player, game, position, skill)`. Source specifies no re-analysis semantics; this is the smallest key that makes re-analysis safe without discarding legitimate repeat observations across different games.
...
None of the above are contradictions affecting architecture, data semantics, security posture, or acceptance. None block this plan.
...
| In / Out of scope | In: Alembic bootstrap and an upgrade path for existing SQLite data; `POST /v1/games` requires an existing `player_id` and writes `Game.owner_player_id`; the PGN import client supplies the active player identity; `run_analysis_for_game` derives player attribution from that owner instead of the `"system"` literal; persists `Position` and `EngineAnalysis` for fast-pass candidates and focused-pass positions; persists the `Evidence` it already constructs and discards; `PersistedLessonOpportunity` gains a player column with a migration. Out: owner reassignment for pre-M31 records, new detectors (M33), profile mutation (M34), auto-triggering (M32), retention/compaction implementation (§2 `> CORRECTION:`). |
| Depends on | `none` |
| Target release | `unversioned — learning loop closed` |
| Deliverables | Alembic configuration, baseline, and SQLite legacy-upgrade test; `Game.owner_player_id` column plus migration and owner-required import contract; PGN import client identity handoff and component coverage; rewritten `src/scan64/chess/analysis/jobs.py` writing all three entity types inside the job's session; player column plus migration on `PersistedLessonOpportunity`; owner-based evidence query in `api/reports.py`; `tests/integration/test_analysis_persistence.py`. |
| Acceptance | After a job on an imported game owned by player P: `GET /v1/games/{id}/positions` returns one row per persisted position with its engine analysis attached; `GET /v1/players/P/evidence` returns the evidence backing every persisted lesson; `rg 'player_id="system"' src/` returns no match; no code path constructs an `Evidence` object it does not persist. An ownerless legacy game is rejected before analysis rather than attributed to a synthetic player. |
| Verification | `uv run pytest tests/integration/test_schema_migrations.py tests/integration/test_analysis_persistence.py` — passes; `pnpm --dir apps/scan64-web test` — passes; `scripts/check.sh` — exits 0. Manual: import a PGN as the active player, poll the job to `completed`, assert both endpoints non-empty. |
...
| Risks & rollback | Risk: persisting analysis for every ply inflates the SQLite file on bulk import — bounded by persisting only candidate and focused-pass positions. Risk: a required import owner breaks the sole UI caller — land its identity handoff with the API contract and verify it with the frontend suite. Risk: ownerless historical rows cannot be safely attributed — leave them immutable, exclude them from player-owned paths, and fail loudly before analysis. Risk: schema changes against existing SQLite files are silent without a real migrator — the legacy-upgrade fixture is mandatory. Rollback: the stack is the rollback unit; no consumer depends on the rows before M34. |
| Est. PRs | 5 |
...
| In / Out of scope | In: the analysis job resolves detectors through `learning.plugins.registry` rather than constructing one concretely; every seeded detector registers at app startup; `FocusedPassOrchestrator` runs on fast-pass candidates and its MultiPV output becomes diagnosis evidence; arbitration when several detectors fire on one position. Out: new detector classes; learned detectors (§2 ASSUMPTION). |
| Depends on | M31 |
| Target release | `unversioned — learning loop closed` |
| Deliverables | Registry bootstrap bound to the app lifespan; two-pass pipeline in `jobs.py` using M31's game-owner attribution; documented arbitration rule with a deterministic tie-break; `tests/integration/test_live_detector_coverage.py` driving the golden corpus through the production job path rather than the benchmark harness. |
| Acceptance | The golden corpus routed through the production job path yields at least one diagnosis for each of the ten seeded codes. A position where two detectors fire produces exactly one primary diagnosis with the loser recorded as secondary. `FocusedPassOrchestrator` is reached from a production call path. Detector precision on the golden corpus is reported and no code regresses below the single-detector baseline's precision on its own positions. |
| Verification | `uv run pytest tests/integration/test_live_detector_coverage.py tests/integration/test_stockfish_pipeline.py` — passes with the precision report captured; `scripts/check.sh` — exits 0. |
| Design reevaluation | Inspect `jobs.py:54-92`, `chess/analysis/orchestration.py:101-117`, `learning/plugins/registry.py:38-64`, `benchmarks/diagnosis_report.py:5-54`, and M31's game-owner contract; confirm G6 and G7 still hold, the benchmark harness's detector assembly is still the only multi-detector construction, and diagnosis context uses the durable game owner. Dependent milestones requiring review: M34, M35, and transitively M36, M37, M38, M43. |
| Risks & rollback | Risk: enabling nine more detectors surfaces false positives the single-detector baseline hid — gate each detector on its taxonomy-declared minimum engine evidence and report precision in the verification output. Risk: the focused pass multiplies analysis cost per game — bound by the existing per-game candidate limit. Rollback: registry lookup is one call site; reverting to the concrete detector restores M31 behaviour. |
| Est. PRs | 5 |
...
| In / Out of scope | In: a single profile-update service used by both the M31/M33 analysis path and the content-attempt path; `SkillState` observation applied from player-attributed analysis findings; rating-band priors applied on first observation of a concept; `ReviewSchedule` created when a lesson is generated and updated when it is attempted; an idempotency key preventing double-counting on re-analysis; a live taxonomy-migration hook so a renamed or retired `skill_id` remaps the `SkillState` and `ReviewSchedule` rows this milestone starts writing. Out: the reports that read this state (M36); adaptive selection (M38). |
| Depends on | M32, M33 |
| Target release | `unversioned — learning loop closed` |
| Deliverables | Profile-update service replacing the duplicated logic in `content/tracking.py:7-40`; prior selection from `PlayerProfile.rating` via `learning/profiling/priors.py`; schedule writer; taxonomy-migration hook invoking `learning/diagnosis/taxonomy/migration.py` over live rows at startup; `tests/integration/test_profile_updates_from_analysis.py`; `tests/property/test_mastery_monotonicity.py`; `tests/integration/test_taxonomy_migration_live.py`. |
| Acceptance | Two games containing the same diagnosis lower that concept's expected mastery monotonically and narrow its uncertainty. A player rated 1200 and a player rated 1900 start a new concept at different priors, neither at `(1.0, 1.0)`. Every generated lesson has a `ReviewSchedule` row with a due date. Re-analysing the same game does not change mastery a second time. A `skill_id` rename applied through the migration table remaps live `SkillState` and `ReviewSchedule` rows; an unmappable code is retired with a recorded reason, never silently dropped. |
| Verification | `uv run pytest tests/integration/test_profile_updates_from_analysis.py tests/property/test_mastery_monotonicity.py tests/integration/test_taxonomy_migration_live.py` — passes; `scripts/check.sh` — exits 0. |
| Design reevaluation | Inspect `content/tracking.py:7-40`, `learning/profiling/priors.py:3-20`, `api/learning.py:199-230`, `learning/diagnosis/taxonomy/migration.py:11-51`, and M31–M33's merged owner-attributed paths; confirm G5, G14, G15, and G31 still hold and that analysis findings carry a real player rather than a synthetic identity. Dependent milestones requiring review: M36, M37, and transitively M38, M43. |
| Risks & rollback | Risk: double-counting on re-analysis — addressed by the idempotency key, which must be asserted in tests, not assumed. Risk: unifying two update paths changes famous-game attempt behaviour — cover the existing path with a characterisation test before refactoring. Risk: a migration hook that runs at startup can corrupt live rows on a bad table — require a no-op default and assert idempotency of the migration itself. Rollback: profile updates are additive and the idempotency key makes recomputation safe. |
| Est. PRs | 6 |
...
| In / Out of scope | In: a template per seeded taxonomy code parameterised by the diagnosis's evidence payload; removal of the catch-all fallback in favour of a conformance failure when a code has no template; the optional LLM path routed through `explanations/validator.py` so generated text is checked against evidence before display, off by default. Out: any hosted LLM dependency in the default install; new taxonomy codes. |
| Depends on | M33 |
| Target release | `unversioned — learning loop closed` |
| Deliverables | Expanded `explanations/templates/provider.py` with per-code templates and evidence interpolation; `tests/conformance/test_explanation_coverage.py` enumerating the taxonomy; LLM path wired behind explicit configuration with grounding validation mandatory when enabled; `tests/unit/test_grounded_explanation.py`. |
| Acceptance | For each of the ten seeded codes the generated explanation names the specific square or piece drawn from its evidence payload. A taxonomy code without a template fails the conformance test rather than rendering the fallback. With the LLM path enabled, an ungrounded generation is rejected and the template output is used instead. The default install acquires no model dependency. |
...
| Design reevaluation | Inspect `explanations/templates/provider.py:3-25`, `explanations/validator.py:55-65`, `providers/llm/config.py:79-106`, and M33's merged detector set; confirm G8 still holds and that the live diagnosis codes match the templates being written. Dependent milestones requiring review: M37. |
...
| In / Out of scope | In: `recurring_habits` computed by `learning/profiling/habits.py` over the player's persisted diagnoses; the openings report derived from the player's own owner-attributed game corpus with per-family result and error rates; the weekly report as a typed object (games played, concepts observed, mastery deltas, top recurring diagnosis). Out: coach-facing aggregation changes; context-conditioned mastery surfacing. |
| Depends on | M34 |
| Target release | `unversioned — learning loop closed` |
| Deliverables | Habit computation wired into `/v1/players/{id}/patterns`; opening-family classification over the player's games for `/v1/reports/openings`; a typed weekly-report model replacing the string; `tests/integration/test_reports_from_real_data.py`. |
| Acceptance | A player with three games sharing one diagnosis has that pattern in `/patterns` with an occurrence count and evidence references. `/openings` reflects that player's actual opening families. `/reports/weekly` returns a typed object. No handler in `src/scan64/api/reports.py` returns a hardcoded literal collection or string. |
| Verification | `uv run pytest tests/integration/test_reports_from_real_data.py` — passes; `scripts/check.sh` — exits 0. |
| Design reevaluation | Inspect `api/reports.py:125-129` and the weekly handler, `learning/profiling/habits.py:90-156`, M31's owner-based evidence query, and M34's merged profile writes; confirm G10, G11, G12, and G13 still hold. Dependent milestones requiring review: `none`. |
| Risks & rollback | Risk: habit detection over a sparse corpus produces noise — apply a minimum-occurrence threshold and surface it in the response so a caller can distinguish "no habit" from "not enough data". Rollback: the endpoints are read-only. |
| Est. PRs | 3 |
...
| In / Out of scope | In: a shared lesson-board component rendering a `LessonSpec` position with legal-move input, hint ladder, and accept/reject feedback, used by Daily Training, Critical Moment Review, and the Opening Explorer's missions; `POST /v1/lesson-attempts` accepting lesson id, session id, submitted move, elapsed time, and hints used; `StudySession` persisted when `/v1/learning/session` is served; Opening Explorer missions submitted through the same attempt endpoint instead of being evaluated client-side against local seed data; attempt-driven profile update reusing M34's service. Out: the adaptive selection logic consuming these attempts (M38); transfer exercises (M43); replacing the Opening Explorer's seed families with a server-side catalog. |
| Depends on | M34 |
| Target release | `unversioned — learning loop closed` |
| Deliverables | `LessonBoard` React component extracted from the working play-board configuration; `DailyTrainingScreen`, `CriticalMomentReview`, and `OpeningExplorerScreen` missions rebuilt on it; generic attempt endpoint and `StudySession` writer; a Playwright spec driving a real pointer answer. |
| Acceptance | Daily Training presents an interactive board, not a FEN string. Submitting the accepted move is confirmed and recorded; submitting a wrong move consumes an attempt and reveals the next hint. A served training session has a `StudySession` row and every attempt links to it. An attempt changes the relevant `SkillState`. Completing an Opening Explorer mission records an attempt server-side rather than being discarded on navigation. |
| Verification | `uv run pytest tests/integration/test_lesson_attempts.py` — passes; `pnpm --dir apps/scan64-web test` and `pnpm --dir apps/scan64-web test:e2e` — pass, including the new pointer-driven attempt spec; `scripts/check.sh` — exits 0. Manual: complete one Daily Training lesson and one Opening Explorer mission end to end and observe the mastery change in `/v1/players/{id}/progress`. |
| Design reevaluation | Inspect `DailyTrainingScreen.tsx:75-90`, `CriticalMomentReview.tsx:75-129`, `OpeningExplorerScreen.tsx:13-126`, `api/learning.py:250-257`, `content/models.py:39-68`, and the merged play-board configuration in `PlayScreen.tsx`; confirm G17, G18, G19, and G23 still hold. Dependent milestones requiring review: M38, M43. |
| Risks & rollback | Risk: extracting the shared board regresses the play board, which was only recently made usable — extract from the working configuration and cover both boards with pointer-level e2e in the same stack. Risk: three screens plus an endpoint in one stack exceeds a reviewable size — the endpoint and the extraction land first and each screen is its own PR. Rollback: the screens are independent; the extraction PR is separately revertible. |
| Est. PRs | 6 |
...
| Field | Value |
| --- | --- |
| Objective | The training session a learner receives is composed from measured mastery, due reviews, and recent fatigue rather than fixed constants. |
| In / Out of scope | In: `/v1/learning/session` loads `SkillState`, due `ReviewSchedule` rows, recent attempt history, and habit output to compute real priority factors; session fatigue derived from recent attempt volume and accuracy; a documented exploration floor. Out: new exercise types; transfer selection (M43). |
| Depends on | M37 |
| Target release | `unversioned — adaptive and operable` |
| Deliverables | Priority-factor computation replacing the hardcoded block at `api/learning.py:213-230`; documented weighting naming each factor's source; `tests/integration/test_adaptive_session.py`. |
| Acceptance | A player with a low-mastery concept receives that concept's lessons ahead of a high-mastery concept's. Overdue reviews outrank exploration items. After a long high-error session, fatigue measurably shifts composition. No constant priority factor remains in the request path. The exploration floor guarantees at least one non-weakness item per session. |
| Verification | `uv run pytest tests/integration/test_adaptive_session.py` — passes; `scripts/check.sh` — exits 0. |
| Design reevaluation | Inspect `api/learning.py:199-257` and M34/M37's merged writes; confirm G16 still holds and that `SkillState` and `ReviewSchedule` now carry the data this milestone depends on. Dependent milestones requiring review: M43. |
...
| Field | Value |
| --- | --- |
| Objective | Export, import, and deletion cover every table holding player-derived data, including M31's `Game.owner_player_id` attribution and the evidence it begins writing. |
| In / Out of scope | In: `Evidence`, `Position`, `EngineAnalysis`, `TransferPosition`, `TransferMeasurement`, and `StudySession` added to the archive schema, the import path, and the deletion path with audit coverage; an enumeration-based completeness test that fails when a player-scoped table is absent from the lifecycle. Out: hosted-mode retention policy (§2 `> GAP:`). |
| Depends on | M31 |
| Target release | `unversioned — adaptive and operable` |
| Deliverables | Extended archive model and handlers in `api/data_lifecycle.py` preserving `Game.owner_player_id` plus `Evidence`, `Position`, `EngineAnalysis`, `TransferPosition`, `TransferMeasurement`, and `StudySession`; `tests/integration/test_data_lifecycle_completeness.py` enumerating SQLModel tables; deletion audit updated to record the added tables. |
| Acceptance | Export, delete, then import round-trips a player with analysed games and leaves no orphan rows in any table. After deletion, every player-scoped table has zero residual rows for that player — asserted per table, not inferred from a success status. Adding a new player-scoped table without registering it fails the completeness test. |
...
| Design reevaluation | Inspect `api/data_lifecycle.py:23-46,118-130,353-365`, the `DeletionAudit` writer at `:367-378`, and M31's merged owner and evidence writes; confirm G28 still holds and that the set of player-scoped tables and owner-bearing fields has not changed. Dependent milestones requiring review: `none`. |
...
| Design reevaluation | Inspect `learning/evaluation/transfer_measurement.py:41-162`, `learning/exercises/transfer.py:13-141`, `learning/verification/verifier.py:26-40`, and M37/M38's merged attempt and session paths; confirm G29 and G30 still hold. Dependent milestones requiring review: `none`. |
