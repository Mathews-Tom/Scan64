...
| `unversioned — adaptive and operable` | `M38, M39, M40, M41, M42, M43, M44, M45` | All included milestones are externally merged. | `none` — see §2 `> GAP:` on release policy | `scripts/check.sh` exits 0 on `main` after the final merge, plus M44's clean-clone quickstart and M45's coach-mode interruption walkthrough | `not requested` |
