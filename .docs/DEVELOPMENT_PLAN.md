...
| Design reevaluation | Inspect `DailyTrainingScreen.tsx:75-90`, `CriticalMomentReview.tsx:75-129`, `OpeningExplorerScreen.tsx:13-126`, `api/learning.py:199-230`, `content/models.py:39-68`, the merged M34 profile-update and schedule-identity contracts, and the working play-board configuration in `PlayScreen.tsx`; confirm G15, G17, G18, G19, and G23 still hold, that the historic `api/content.py` G15 pointer remains closed because content items have no M34 schedule identity, that attempts retain learner work without reactivating a retired taxonomy skill, and that the submitted lesson id resolves an owned `PersistedLessonOpportunity` before its schedule is loaded. Dependent milestones requiring review: M38, M43. |
| Risks & rollback | Risk: extracting the shared board regresses the play board, which was only recently made usable — extract from the working configuration and cover both boards with pointer-level e2e in the same stack. Risk: three screens plus an endpoint in one stack exceeds a reviewable size — the endpoint and the extraction land first and each screen is its own PR. Risk: generic attempts advance a schedule in the wrong key space — resolve the owned persisted lesson first, then its schedule, and cover it with the endpoint integration test. Rollback: the screens are independent; the extraction PR is separately revertible. |
| Est. PRs | 6 |
