...
3. Revalidate objective, interfaces, dependencies, acceptance, verification, risks, release train, and dependent milestone listed in M36's Design reevaluation row (M38).
4. Append one ledger entry to .docs/DEVELOPMENT_PLAN_HISTORY.md: timestamp, milestone, decision, trigger, evidence, plan/prompt sections changed, downstream impact, implementation authorization.
5. If no material mismatch exists, report `DESIGN GO - PLAN REVISION: none`; this authorizes implementation.
...
CONTEXT: DEVELOPMENT_PLAN.md §6 Section G M37 + source .docs/2026-07-26-learning-loop-enhancement-plan.md §2.2 (G15), §2.3 (G17-G19, G23) and .docs/scan64-system-design.md §10 (exercises), §12.5 (opening missions), §20 (application screens), §21 (scheduler). Preconditions: M34 merged. Repo: Python 3.12+/uv backend; React 19 + TypeScript + Vite + pnpm frontend with chessground and chess.js; Vitest, Playwright; scripts/check.sh.
OBJECTIVE: A learner can attempt any generated lesson on a real board and the attempt is recorded against their profile. Acceptance: Daily Training presents an interactive board only for a player-owned persisted opportunity and an explicit no-eligible-lessons state otherwise; a persisted-opportunity accepted move is server-verified, confirmed, and recorded; a wrong move consumes an attempt and reveals the next hint; a served training session has a StudySession row, returns its id, and every LessonAttempt links to it; a persisted-opportunity attempt resolves an owned PersistedLessonOpportunity and advances its matching M34 ReviewSchedule, changes the relevant active SkillState, and cannot recreate a retired code; an Opening Explorer local-seed mission records an explicitly ungraded server-side attempt rather than being discarded on navigation.
...
2. Inspect apps/scan64-web/src/components/DailyTrainingScreen.tsx:75-90, CriticalMomentReview.tsx:75-129, PlayScreen.tsx's working chessground configuration including its bounds recompute, src/scan64/api/learning.py:154-290, src/scan64/content/models.py:39-68, `PersistedLessonOpportunity`, and M34's merged profile-update service with active/retired taxonomy state. Confirm that ContentAttempt.item_id has a ContentItem foreign key while M34 schedules use PersistedLessonOpportunity ids, that the generic attempt path therefore needs a distinct typed record, and that every Daily Training lesson reaching the profile-recording board has an owned persisted-opportunity id.
3. Revalidate objective, interfaces, dependencies, acceptance, verification, risks, release train, and every dependent milestone listed in M37's Design reevaluation row (M38, M42, M43).
4. Append one ledger entry to .docs/DEVELOPMENT_PLAN_HISTORY.md: timestamp, milestone, decision, trigger, evidence, plan/prompt sections changed, downstream impact, implementation authorization.
5. If no material mismatch exists, report `DESIGN GO - PLAN REVISION: none`; this authorizes implementation.
...
1. PR-1 Generic attempt endpoint and StudySession - scope: add player-scoped LessonAttempt separate from ContentAttempt, with a required StudySession relation, lesson id, source kind, submitted move, elapsed time, hints used, grading status, and explicit profile-update result; persist a StudySession when /v1/learning/session is served and return its id with the lessons; serve profile-recording Daily Training only from player-owned PersistedLessonOpportunity rows, returning an explicit empty session when none are eligible; POST /v1/lesson-attempts resolves only persisted_opportunity ids to an owned PersistedLessonOpportunity before server-verifying the stored LessonSpec, loading and advancing its M34 ReviewSchedule, and reusing M34's active/retired profile-update service; opening_mission records the local-seed mission as ungraded and never advances a schedule or profile; commits: "feat(api): persist a study session for every served training session", "feat(api): record a generic lesson attempt"; verification: `uv run pytest tests/integration/test_lesson_attempts.py`
...
3. PR-3 Daily Training on the lesson board (on PR-2) - scope: replace the raw-FEN rendering with the board, hint ladder, accept/reject feedback wired to the attempt endpoint, and an explicit no-eligible-lessons state; commits: "feat(web): make daily training interactive"; verification: `pnpm --dir apps/scan64-web test`
...
5. PR-5 Opening Explorer missions on the lesson board (on PR-4) - scope: submit local-seed mission completions through the generic attempt endpoint as ungraded opening_mission attempts instead of discarding them on navigation; do not introduce a server-side opening catalog or claim server-verified mission correctness; commits: "feat(web): record opening-mission attempts server-side"; verification: `pnpm --dir apps/scan64-web test`
...
CONSTRAINTS: no scope leakage, minimal dependencies, repo style, no version/changelog updates. Extract the shared board from the play board's working configuration - do not re-derive chessground setup, and keep the bounds recompute that makes the board usable. M37 establishes the Daily Training source-eligibility boundary required for durable attempt identity; M38 owns later adaptive priority and composition from mastery, due schedules, and attempt history. Do not replace the Opening Explorer's seed families with a server-side catalog; only its ungraded attempt recording moves server-side. Do not silently recreate a taxonomy skill retired by M34 or discard the learner's valid attempt. Advance schedules only for persisted_opportunity attempts after resolving the submitted canonical id to an owned PersistedLessonOpportunity and then loading its schedule; do not look up M34 schedules from the famous-game content-attempt endpoint. Keep ContentAttempt and the famous-game route unchanged.
ATTEMPT-CONTEXT CONTRACT: Every Daily Training lesson that can submit a persisted_opportunity attempt must carry a player-owned canonical opportunity id; static catalog lessons must not reach that board as an ungraded fallback. Critical Moment Review product work is blocked until its serving path provides a durable `StudySession.id` and canonical owned persisted-opportunity id. Opening Explorer must receive a durable session id but records only ungraded `opening_mission` attempts. M38 exclusively owns shared-session composition changes after M37 has established source eligibility.
VERIFICATION (must pass): `uv run pytest tests/integration/test_lesson_attempts.py` passes, including session linkage, Daily Training source eligibility, separate attempt identity, persisted-opportunity verification and schedule/profile updates, retired-code skip, and ungraded opening-mission recording; `pnpm --dir apps/scan64-web test` and `pnpm --dir apps/scan64-web test:e2e` pass, including the no-eligible-lessons state and pointer-driven attempt spec; `scripts/check.sh` exits 0; manual - complete one Daily Training persisted-opportunity lesson and one Opening Explorer mission end to end and observe the mastery and matching schedule change in GET /v1/players/{id}/progress.
REVIEW:
Per PR:
- Scope matches its purpose; contracts match the reconciled plan; behavior is meaningfully tested.
