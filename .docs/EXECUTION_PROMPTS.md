...
2. Inspect apps/scan64-web/src/components/DailyTrainingScreen.tsx:75-90, CriticalMomentReview.tsx:75-129, PlayScreen.tsx's working chessground configuration including its bounds recompute, src/scan64/api/learning.py:199-257, src/scan64/content/models.py:39-68, and M34's merged profile-update service with active/retired taxonomy state.
...
1. PR-1 Generic attempt endpoint and StudySession - scope: POST /v1/lesson-attempts accepting lesson id, session id, submitted move, elapsed time, and hints used; persist a StudySession when /v1/learning/session is served; resolve the submitted canonical id to an owned PersistedLessonOpportunity before loading and advancing its M34 ReviewSchedule; reuse M34's profile-update service; record an attempt for a retired skill with an explicit skipped-profile result rather than recreating or rejecting it; commits: "feat(api): persist a study session for every served training session", "feat(api): record a generic lesson attempt"; verification: `uv run pytest tests/integration/test_lesson_attempts.py`
...
CONSTRAINTS: no scope leakage, minimal dependencies, repo style, no version/changelog updates. Extract the shared board from the play board's working configuration - do not re-derive chessground setup, and keep the bounds recompute that makes the board usable. Do not change session composition; that is M38. Do not replace the Opening Explorer's seed families with a server-side catalog; only the attempt submission moves server-side. Do not silently recreate a taxonomy skill retired by M34 or discard the learner's valid attempt. Advance schedules only after resolving the submitted canonical id to an owned PersistedLessonOpportunity and then loading its schedule; do not look up M34 schedules from the famous-game content-attempt endpoint.
