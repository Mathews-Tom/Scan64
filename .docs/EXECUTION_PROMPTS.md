...
OBJECTIVE: A learner can attempt any generated lesson on a real board and the attempt is recorded against their profile. Acceptance: Daily Training presents an interactive board, not a FEN string; a persisted-opportunity accepted move is server-verified, confirmed, and recorded; a wrong move consumes an attempt and reveals the next hint; a served training session has a StudySession row, returns its id, and every LessonAttempt links to it; a persisted-opportunity attempt resolves an owned PersistedLessonOpportunity and advances its matching M34 ReviewSchedule, changes the relevant active SkillState, and cannot recreate a retired code; an Opening Explorer local-seed mission records an explicitly ungraded server-side attempt rather than being discarded on navigation.
...
2. Inspect apps/scan64-web/src/components/DailyTrainingScreen.tsx:75-90, CriticalMomentReview.tsx:75-129, PlayScreen.tsx's working chessground configuration including its bounds recompute, src/scan64/api/learning.py:199-257, src/scan64/content/models.py:39-68, and M34's merged profile-update service with active/retired taxonomy state. Confirm that ContentAttempt.item_id has a ContentItem foreign key while M34 schedules use PersistedLessonOpportunity ids, and that the generic attempt path therefore needs a distinct typed record.
...
CONSTRAINTS: no scope leakage, minimal dependencies, repo style, no version/changelog updates. Extract the shared board from the play board's working configuration - do not re-derive chessground setup, and keep the bounds recompute that makes the board usable. Do not change session composition; that is M38. Do not replace the Opening Explorer's seed families with a server-side catalog; only its ungraded attempt recording moves server-side. Do not silently recreate a taxonomy skill retired by M34 or discard the learner's valid attempt. Advance schedules only for persisted_opportunity attempts after resolving the submitted canonical id to an owned PersistedLessonOpportunity and then loading its schedule; do not look up M34 schedules from the famous-game content-attempt endpoint. Keep ContentAttempt and the famous-game route unchanged.
ATTEMPT-CONTEXT CONTRACT: Critical Moment Review product work is blocked until its serving path provides a durable `StudySession.id` and canonical owned persisted-opportunity id. Opening Explorer must receive a durable session id but records only ungraded `opening_mission` attempts. M38 exclusively owns shared-session composition changes.
VERIFICATION (must pass): `uv run pytest tests/integration/test_lesson_attempts.py` passes, including session linkage, separate attempt identity, persisted-opportunity verification and schedule/profile updates, retired-code skip, and ungraded opening-mission recording; `pnpm --dir apps/scan64-web test` and `pnpm --dir apps/scan64-web test:e2e` pass; `scripts/check.sh` exits 0; manual - complete one Daily Training persisted-opportunity lesson and one Opening Explorer mission end to end and observe the mastery and matching schedule change in GET /v1/players/{id}/progress.
REVIEW:
Per PR:
- Scope matches its purpose; contracts match the reconciled plan; behavior is meaningfully tested.
...
2. Inspect src/scan64/api/learning.py:199-257 and the merged M34/M37 writes to active SkillState, non-retired ReviewSchedule, typed LessonAttempt, and StudySession, with their CI evidence.
...
2. Inspect src/scan64/learning/evaluation/transfer_measurement.py:41-162, src/scan64/learning/exercises/transfer.py:13-141, src/scan64/learning/verification/verifier.py:26-40, and the merged M37/M38 typed attempt and active-session paths with their CI evidence. Confirm transfer completion remains distinct from M37's ungraded opening_mission records.
