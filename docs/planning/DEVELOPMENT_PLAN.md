...
| §6 Section H (Make the learner able to act, M38–M40 and M45) | `docs/planning/learning-loop-enhancement-plan.md` §2.2 (G16), §2.3 (G20–G22); `docs/planning/system-design.md` §10 (exercises), §20 (application screens), §21 (scheduler) |
...
| In / Out of scope | In: consuming M31's persisted `EngineAnalysis` through M39's owner-authorized analysis route in `AnalysisScreen`, per-position evaluation display, diagnosis markers on the move list, and distinct states for "not analysed yet", "analysis found nothing", and H-033 not-found. Out: on-demand interactive engine analysis of arbitrary user-entered positions or owner reassignment. |
| Depends on | M31, M39 |
| Target release | `v0.2.0 — adaptive and operable` |
| Deliverables | Evaluation rendering and diagnosis markers in `AnalysisScreen.tsx`; an "analyse this game" action for an unanalysed owned game; H-033 not-found handling for missing, malformed, ownerless, and non-owned resources; authenticated component and API coverage. |
| Acceptance | Opening an owned analysed game shows an evaluation for every persisted position and a marker at each diagnosed position. An unanalysed owned game offers an analyse action rather than a dead message. A game analysed with no findings says so explicitly. Missing, malformed, ownerless, and non-owned routes render not-found without an action or data disclosure. |
| Verification | `uv run pytest tests/integration/test_analysis_board_api.py` — proves owner authorization; `pnpm --dir apps/scan64-web test` — passes; `scripts/check.sh` — exits 0. Manual: open a locally analysed owned game and confirm evaluations and markers render. |
| Design reevaluation | Inspect `AnalysisScreen.tsx:32-183`, M39's merged H-033 route and bearer-header behavior, and M31's `/positions` payload; confirm G22 still holds and that the screen distinguishes analysis state from H-033 not-found. Dependent milestones requiring review: `none`. |
| Risks & rollback | Risk: the analysis screen can regress to a public or ambiguous empty state. Rollback: a single screen after retaining the H-033 backend boundary. |
| Est. PRs | 2 |
