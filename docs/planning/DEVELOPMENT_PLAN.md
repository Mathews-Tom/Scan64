...
At the `f616763`/`6da18dd` baseline, Scan64's first thirty milestones were merged and the repository quality gate passed, but the gate measured each milestone in isolation and nothing measured the seam between subsystems. A live audit of the running stack — uvicorn plus the production web bundle, real Stockfish, real browser pointer input — established that the product's central promise (*play, get diagnosed, train your weaknesses*) was not connected end to end. This plan covers only the milestones that close that baseline gap. M1–M30 behavior is established by merged Git history; this file supersedes its historical local planning artifacts as the authoritative plan for active work. M31–M38 subsequently merged and `v0.1.0` shipped on 2026-07-28; M39 is next.

The tracked documents in `docs/planning/` are the authoritative planning set. `docs/planning/learning-loop-enhancement-plan.md` supplies the observed gap inventory (G1–G33), established by direct observation of a running system and two read-only source sweeps with `file:line` evidence against `f616763`; `docs/planning/system-design.md` is the requirements proposal for *what the behaviour should be*. `docs/system-design.md` is a separate architecture companion. The former `.docs/` directory is intentionally local and ignored.

| Plan section | Source document / section |
| --- | --- |
| §6 Section G (Close the diagnostic loop, M31–M37) | `docs/planning/learning-loop-enhancement-plan.md` §2.1 (G1–G9), §2.2 (G10–G15, G17), §2.3 (G18, G19, G23), §2.4 (G31); `docs/planning/system-design.md` §7 (learning pipeline), §8 (taxonomy), §9 (player model), §14 (LLM integration), §17 (data model) |
| §6 Section H (Make the learner able to act, M38–M40 and M45) | `docs/planning/learning-loop-enhancement-plan.md` §2.2 (G16), §2.3 (G20–G22); `docs/planning/system-design.md` §7.4 (diagnosis provenance), §10 (exercises), §17.2 (diagnosis retention), §20 (application screens), §21 (scheduler) |
| §6 Section I (Runtime integrity and operations, M41–M44) | `docs/planning/learning-loop-enhancement-plan.md` §2.3 (G24), §2.4 (G26–G30, G32–G33); `docs/planning/system-design.md` §18.6 (compute budgets), §22 (testing), §24.1 (privacy/export/deletion), §23.4–23.5 (learning metrics) |
| §4 Release Trains | Repository evidence: `pyproject.toml:3` (`version = "0.1.0"`); annotated `v0.1.0` tag; published GitHub Release and PyPI package; tracked `CHANGELOG.md` with released `[0.1.0]` notes |
| §7 Cross-Cutting Concerns | `docs/planning/learning-loop-enhancement-plan.md` §3 (sequencing principle); `.github/workflows/ci.yml`; branch ruleset on `main` |
...
> DECISION (H-033): M39 turns every player-reachable game and play-session route into an owner-authorized boundary. Until M39 merges, those routes remain a named exposure. The caller supplies a bearer token for the already-authorized active player; session creation, session reads, moves, resignation, game collection/listing, game reads, position reads, and analysis-job reads and writes reject missing or mismatched ownership without disclosing game data. An ownerless legacy game is unreachable from this surface and yields the same not-found response as a nonexistent or non-owned resource. At request time, `PlaySession.player_id` authorizes the play-session resource itself but never establishes `Game` ownership or substitutes for `Game.owner_player_id`.
> DECISION (H-003): M31 bootstraps Alembic and supplies an upgrade path for existing SQLite databases before adding owner and lesson-opportunity columns. A populated legacy SQLite fixture must prove both one-time session-backed owner backfill and null ownership where no session exists before any M31 query runs. `docs/planning/system-design.md` §17.3 also calls for PostgreSQL migration CI; the current SQLite-only repository has no PostgreSQL service or driver, so M31 records that unimplemented cross-database verification as a visible future-planning gap rather than claiming it.
...
> GAP (H-018): G25's missing form labels and keyboard board path is unassigned. M39 must not absorb it: history navigation does not establish an accessible board interaction contract. A future accessibility milestone must define, implement, and verify both requirements.
...
graph TD
 ...
 M32 --> M41
  ...
  M37 --> M45[M45 coach-mode interruptions]
  M41 --> M45
```
...
| --- | --- | --- | --- | --- | --- |
| `v0.1.0 — learning loop closed` | `M31, M32, M33, M34, M35, M36, M37` | Released 2026-07-28. | Released `CHANGELOG.md` notes, verified `uv build` artifacts, clean-install `scan64-cli` smoke test, and prior public release of `chess-lesson-spec==0.1.0` | `scripts/check.sh` passed on `main` after the final merge, plus M37's Daily Training, game-analysis review, and Opening Explorer walkthroughs | Published with explicit maintainer authorization |
...
- The tracked plan and prompt files live in `docs/planning/`; `.docs/DEVELOPMENT_PLAN_HISTORY.md` is reconstructible local evidence and is ignored by git.
...
| Design reevaluation | Inspect `content/tracking.py:7-40`, `learning/profiling/priors.py:3-20`, `api/learning.py:199-230`, `learning/diagnosis/taxonomy/migration.py:11-51`, the `SkillState`, `ReviewSchedule`, and `ProfileObservation` schemas, the Alembic chain, and M31–M33's merged owner-attributed paths; confirm G5, G14, G15, and G31 still hold, that analysis findings carry a real player and game rather than synthetic or nullable identities, that `ReviewSchedule.item_id` is canonical `str(PersistedLessonOpportunity.id)` M37 will submit, and that a taxonomy rename cannot bypass the active observation idempotency key. Dependent milestones requiring review: M36, M37, M42, and transitively M38, M43, M45. |
...
| In / Out of scope | In: URL routing replacing the single `currentView` state; a paginated games-list screen backed by M32's owner-based `GET /v1/players/{id}/games` showing result, nullable PGN game date, and diagnosis count; deep links to an owned game's existing analysis screen; in-progress play-session restoration on reload; H-033 owner authorization on every player-reachable session, game collection/listing, game, position, and analysis-job read or write. Out: PGN export UI beyond what the data-lifecycle endpoints already provide; evaluation and diagnosis rendering (M40). |
| Depends on | M32 |
| Target release | `v0.2.0 — adaptive and operable` |
| Deliverables | Router in `App.tsx` landed before any screen changes; `GamesListScreen` with cursor pagination and an explicit unknown-date state; play-session resumption from a persisted session id; per-game analysis route; H-033 bearer authorization and identical not-found handling for missing, malformed, ownerless, and non-owned session/game/position/analysis-job resources, including analysis-job creation; frontend requests use only the already-authorized active player identity and never create or substitute one. |
| Acceptance | Navigating away from an active game and back resumes the same position. The games list shows every game durably owned by the player, whether played or imported after M31, with every cursor page reachable; it displays the PGN game date when present and an explicit unknown-date state otherwise, never an import timestamp. It requests only the already-authorized active player identity. Missing and cross-player analysis-job reads and creation return not-found without disclosure or enqueueing work. A malformed, nonexistent, ownerless, or non-owned game-analysis URL renders not-found without disclosing game data; an owned game's existing analysis view is reachable by URL. A browser reload during an owned game resumes rather than restarting. |
| Verification | `uv run pytest tests/integration/test_play_api.py tests/integration/test_analysis_board_api.py tests/integration/test_player_games.py` — proves missing and cross-player authorization, including analysis-job reads and creation, is rejected on H-033 routes and nullable date semantics are preserved; `pnpm --dir apps/scan64-web test` and `pnpm --dir apps/scan64-web test:e2e` — pass, including active-player identity, unknown-date, deep-link not-found, pagination, and navigate-away-and-resume coverage; `scripts/check.sh` — exits 0. |
| Design reevaluation | Inspect `App.tsx`, every screen's local state, the PGN import identity handoff, the frontend authorization helpers, and M32's merged owner-based `GET /v1/players/{id}/games`; confirm G20 and G21 still hold without identity substitution or owner-data disclosure. Dependent milestones requiring review: M40, M44, M45. |
| Risks & rollback | Risk: routing touches every screen at once and H-033 makes previously public routes owner boundaries — land the router with existing screens unchanged in the first PR, then add resumption, list, and deep links with their authorization contract. Rollback: the router PR is independently revertible; do not roll back owner checks independently of the UI routes that depend on them. |
| Est. PRs | 4 |
...
| Field | Value |
| --- | --- |
| Objective | The analysis board renders persisted engine evaluations and diagnoses instead of reporting that no analysis is available, with every diagnosis bound to its source position. |
| In / Out of scope | In: a migration-backed `PersistedLessonOpportunity.source_position_id` foreign key to the pre-move `Position`; owner-authorized diagnosis and analysis-status projections for `AnalysisScreen`; per-position evaluation display; diagnosis markers on the move list; and distinct states for "not analysed yet", "analysis found nothing", and H-033 not-found. Out: on-demand interactive engine analysis of arbitrary user-entered positions, owner reassignment, and inferred diagnosis-to-position links from FEN at read time. |
| Depends on | M31, M39 |
| Target release | `v0.2.0 — adaptive and operable` |
| Deliverables | A migration that backfills every existing persisted opportunity only when its serialized source FEN resolves to exactly one position in the same game and otherwise fails loudly; new analysis writes that persist the pre-move source-position foreign key; owner-authorized game analysis status and position diagnosis projections; evaluation rendering and diagnosis markers in `AnalysisScreen.tsx`; an "analyse this game" action for an unanalysed owned game; and H-033 not-found handling for missing, malformed, ownerless, and non-owned resources. |
| Acceptance | Opening an owned analysed game shows an evaluation for every persisted position and a marker for every persisted diagnosis at its durable source position. An unanalysed owned game offers an analyse action rather than a dead message. A completed analysis with no persisted diagnoses says so explicitly. Missing, malformed, ownerless, and non-owned routes render not-found without an action or data disclosure. |
| Verification | `uv run pytest tests/integration/test_schema_migrations.py tests/integration/test_analysis_persistence.py tests/integration/test_analysis_board_api.py` — passes, proving the non-guessing backfill, source-position persistence, owner authorization, analysis-state distinction, and diagnosis projection; `pnpm --dir apps/scan64-web test` — passes; `scripts/check.sh` — exits 0. Manual: open a locally analysed owned game and confirm evaluations and source-position markers render. |
| Design reevaluation | Inspect `AnalysisScreen.tsx:29-257`, M39's merged H-033 bearer-header behavior, M31's `/v1/games/{id}/positions` payload, `PersistedLessonOpportunity`, the Alembic chain, and the analysis worker; confirm G22's residual UI half holds — an unanalysed owned game still renders a dead "No engine analysis available" message and no diagnosis markers exist, while M31 already makes per-position evaluations available — H-035 fully and implementably addresses the currently absent source-position relation and diagnosis/status projections, the migration can reject ambiguous legacy rows without a fallback, and the screen can distinguish analysis state from H-033 not-found. Dependent milestones requiring review: `none`. |
| Risks & rollback | Risk: a guessed FEN association silently marks the wrong move, especially after repetitions. The migration must reject unresolved or non-unique legacy links, and runtime reads must use only the durable foreign key. Rollback: revert the two-PR M40 stack as one unit after restoring the migration's documented downgrade path. |
| Est. PRs | 2 |
...
| In / Out of scope | In: a capacity-bounded server producer on the active play path; durable persistence of the generated opportunity, its review schedule, and a `StudySession` before returning a typed interruption payload; an H-033 owner-authorized `PlayMoveResponse` contract; and a pointer-driven coach-mode interruption walkthrough. Out: changing M38 session composition, an always-on engine during ordinary or independent play, and a client-generated lesson or identity. |
| Depends on | M37, M39, M41 |
| Target release | `v0.2.0 — adaptive and operable` |
| Deliverables | A typed `CriticalInterruptionRead` carrying `LessonSpec`, canonical opportunity id, and study-session id; a server-side coach-mode diagnostic and persistence transaction; H-033 authorization and ownership checks on the move path; `PlayScreen` context handoff to `CriticalMomentReview`; integration and pointer-level e2e coverage. |
| Acceptance | With coach mode explicitly enabled, a diagnosed move returns an interruption only after its owned persisted opportunity, schedule, and study session exist; answering the board records a verified attempt against that exact opportunity and updates the matching active profile state. Missing or non-owned play-session writes return the H-033 not-found surface without context disclosure. Ordinary play and independent calculation never invoke the interruption producer. A failed diagnostic or persistence transaction returns no interrupt and no partial attempt context. |
| Verification | `uv run pytest tests/integration/test_coach_interruption_attempts.py`; `pnpm --dir apps/scan64-web test`; `pnpm --dir apps/scan64-web test:e2e`; `scripts/check.sh` — pass. Manual: enable coach mode, trigger a diagnosed interruption, answer it, and inspect the linked attempt and schedule in player progress. |
| Design reevaluation | Inspect M37's context contract, M39's H-033 play authorization, M41's pooled interactive capacity and admission boundary, `PlaySessionService.make_move`, `PlayMoveResponse`, `CriticalMomentReview`, and `docs/planning/system-design.md` §20.3. Confirm a real diagnostic source exists before exposing an interruption. Dependent milestones requiring review: `none`. |
| Risks & rollback | Risk: synchronous diagnostics degrade interactive play — M41's bounded pool and explicit coach opt-in are prerequisites. Risk: returning an interruption before persistence creates orphaned frontend state or crosses an owner boundary — construct the response only after the transaction commits and retain H-033. Rollback: omit the optional response payload and retain normal play. |
| Est. PRs | 4 |
...
| Design reevaluation | Inspect `providers/stockfish/pool.py:78-151`, `chess/analysis/admission.py:14-86`, `api/games.py:158-168`, `chess/opponents/stockfish_opponent.py:22-28`, M31's game-owner contract, and M32's merged auto-enqueue path; confirm G26 and G27 still hold and that M32's in-flight cap is removed rather than coexisting with admission control. Dependent milestones requiring review: M45. |
...
| Field | Value |
| --- | --- |
| Objective | Export, import, and deletion cover every table holding player-derived data, including M31's `Game.owner_player_id` attribution and analysis rows, M34's `ProfileObservation`, M37's `LessonAttempt`, and coach-student relation records. |
| In / Out of scope | In: player-derived `Evidence`, `Position`, `EngineAnalysis`, `ProfileObservation`, `LessonAttempt`, `TransferMeasurement`, `StudySession`, and `CoachStudentLink` covered by the archive schema, import path, and deletion path; global curated `TransferPosition` records included only in the archive/import path to preserve `TransferMeasurement` references and never deleted with an individual player; deletion removes the authenticated player's `IdempotencyRecord` rows and does not cache the terminal deletion response; an enumeration-based completeness test that fails when a player-scoped table is absent from the lifecycle. `DeletionAudit` remains the required non-rehydratable tombstone and is asserted separately from player-scoped application rows. Out: hosted-mode retention policy (§2 `> GAP:`). |
| Depends on | M31, M34 |
| Target release | `v0.2.0 — adaptive and operable` |
| Deliverables | Extended archive model and handlers in `api/data_lifecycle.py` preserving `Game.owner_player_id` plus player-derived `Evidence`, `Position`, `EngineAnalysis`, `ProfileObservation`, `LessonAttempt`, `TransferMeasurement`, `StudySession`, and `CoachStudentLink`; archive/import-only `TransferPosition` references; authenticated-player cleanup of transient idempotency records without caching the terminal delete response; `tests/integration/test_data_lifecycle_completeness.py` enumerating SQLModel tables; deletion audit updated to record the added tables. |
| Acceptance | Export, delete, then import round-trips a player with analysed games, profile observations, lesson attempts, a transfer measurement with its global transfer-position reference, and a coach-student relation against an existing counterpart, leaving no orphan rows in any player-scoped application table. After deletion, every player-scoped application table has zero residual rows for that player — asserted per table, not inferred from a success status; the global `TransferPosition` remains unchanged; the one required `DeletionAudit` tombstone is asserted independently and never rehydrated. Adding a new player-scoped table without registering it fails the completeness test. |
...
| Design reevaluation | Inspect `api/data_lifecycle.py:23-46,118-130,353-365`, the `DeletionAudit` writer at `:367-378`, `api/middleware.py`, the full SQLModel table set, M31's merged owner and evidence writes, M34's `ProfileObservation` schema, M37's player-scoped `LessonAttempt` schema, and `TransferPosition`; confirm G28 still holds, `TransferPosition` remains global while `TransferMeasurement` is player-scoped, shared coach-link restoration keeps the counterpart boundary, and that the set of player-scoped tables and owner-bearing fields has not changed. Dependent milestones requiring review: `none`. |
| Risks & rollback | Risk: deletion coverage bugs are silent — acceptance asserts zero residual rows per table rather than a success status. Risk: deleting a global transfer position with one player's measurement corrupts other learners' data — retain global curated positions and delete only the player's `TransferMeasurement` rows. Risk: a destructive path run unattended during development destroys local data — the human review gate below is mandatory. Risk: retaining an idempotency response or treating an audit tombstone as restored player data defeats deletion — evict the authenticated principal's transient records and assert the tombstone separately. Rollback: additive to the archive schema; deletion changes are the risk and are gated. |
| Est. PRs | 3 |
...
| In / Out of scope | In: Playwright specs driving the board with real pointer input for play, lesson attempts, and analysis; retirement of the `window.__e2e_move` hook as the primary path; authenticated routed resumption and deep-link coverage from M39; `scripts/run.sh` starting API and web together; a README quickstart with prerequisites and the required player-owned PGN import flow; `SCAN64_DATABASE_URL` replacing the bare relative `"database.db"`, defaulting to today's path. Out: hosted deployment; container packaging. |
| Depends on | M39 |
| Target release | `v0.2.0 — adaptive and operable` |
| Deliverables | Rewritten e2e specs using genuine `mouse.down`/`move`/`up` interaction, including authenticated routed play-session resumption and game-analysis deep-link not-found; `scripts/run.sh`; README quickstart naming uv, pnpm, and Stockfish prerequisites; configurable database URL in `persistence/database.py`. |
| Acceptance | An e2e spec fails when the board stops accepting pointer input — demonstrated by reverting the bounds recompute in a scratch worktree and observing the failure, then restoring. An authenticated user can navigate away from and back to a resumed game and open an owned analysis deep link; malformed and non-owned links render not-found. A clean clone reaches a playable board following only the README. Setting `SCAN64_DATABASE_URL` relocates the database and the default preserves current behaviour. The `window.__e2e_move` hook remains only where pointer input is not the subject under test. |
...
| Design reevaluation | Inspect the four existing specs (`lesson.spec.ts`, `review-interaction.spec.ts:43-70`, `analysis-flow.spec.ts`, `offline-smoke.spec.ts:50-56`), `persistence/database.py:5-6`, `README.md`, M31's import-owner contract, and M39's merged routing and H-033 authorization; confirm G24, G32, and G33 still hold. Dependent milestones requiring review: `none`. |
...
- **H-033 read authorization.** M39 remediates the pre-existing exposure by turning player-reachable session, game collection/listing, game, position, and analysis-job reads and writes into owner-authorized boundaries. Ownerless, missing, malformed, and non-owned resources have one not-found surface; M40 renders that state and M44 exercises it with authenticated browser paths.
- **H-035 diagnosis provenance.** M40 repairs the missing durable diagnosis-to-position relation before rendering it: every `PersistedLessonOpportunity` stores its pre-move `source_position_id`, and owner-authorized position reads project diagnoses only through that relation. Legacy backfill must reject ambiguous or unresolved source FENs rather than guessing.
- **Privacy and deletion (`docs/planning/system-design.md` §24.1).** M31 begins persisting evidence that is player-attributed; M42 is therefore not optional cleanup but the milestone that keeps the deletion contract honest once that data exists. The two are on the same dependency edge deliberately.
- **Compute budgets (`docs/planning/system-design.md` §18.6).** M33 adds a second engine pass and M32 adds automatic analysis; M41 is what keeps interactive play responsive once both land. M32's per-player in-flight cap is the interim bound and is removed when M41 supersedes it.
...
| 5 | M38, M39 | Sessions adapt to measured state and history becomes navigable through owner-authorized routes. |
| 6 | M40, M41, M42, M43, M44, M45 | Evaluations are visible, capacity is bounded, player data is portable and deletable, transfer is measurable, interaction is tested, the app is runnable from a clean clone, and coach interruptions have durable evidence. |
