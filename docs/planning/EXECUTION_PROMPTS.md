...
CONTEXT: `docs/planning/DEVELOPMENT_PLAN.md` §6 Section H M40 + source `docs/planning/learning-loop-enhancement-plan.md` §2.3 (G22) and `docs/planning/system-design.md` §§7.4, 17.2, 20 (diagnosis provenance and application screens). Preconditions: M31 and M39 merged. Repo: Python 3.12+/uv, FastAPI + SQLModel + Alembic, pytest/ruff/mypy --strict; React 19 + TypeScript + Vite + pnpm; Vitest, Playwright; scripts/check.sh.
READ AUTHORIZATION AND PROVENANCE CONTRACT (H-033/H-035): `AnalysisScreen` consumes only owner-authorized game and position reads. A persisted diagnosis is rendered only through `PersistedLessonOpportunity.source_position_id`, never by FEN inference at read time. Missing, malformed, ownerless, and non-owned resources render not-found without data disclosure or a doomed action.
OBJECTIVE: The analysis board renders persisted engine evaluations and diagnoses instead of reporting that no analysis is available. Acceptance: an owned analysed game shows an evaluation for every persisted position and a marker for every persisted diagnosis at its durable source position; an unanalysed owned game offers an analyse action rather than a dead message; a completed analysis with no persisted diagnoses says so explicitly; a missing, malformed, ownerless, or non-owned route renders not-found.
RELEASE TRAIN: target=v0.2.0 - "adaptive and operable"; included milestones=M38-M45; preparation trigger=all included milestones externally merged; required artifacts=pyproject.toml version bump, CHANGELOG.md release notes, uv build artifacts, clean-install scan64-cli smoke test; release verification=scripts/check.sh exits 0 on main after the final merge plus M44's clean-clone quickstart and M45's coach-mode interruption walkthrough; publication=explicit maintainer authorization required.
...
2. Inspect `AnalysisScreen.tsx:29-257`, M39's merged H-033 bearer-header behavior, M31's `/v1/games/{id}/positions` payload, the `PersistedLessonOpportunity` model, its Alembic history, and the analysis worker. Confirm G22's residual UI half holds — an unanalysed owned game still renders a dead "No engine analysis available" message and no diagnosis markers exist, while M31 already makes per-position evaluations available — the pre-M40 absence of a durable source-position relation and diagnosis/status projections is exactly the H-035 work now planned; the migration can reject ambiguous legacy rows without a fallback; and the proposed owner-authorized API and UI contracts require no read-time FEN recovery.
...
1. PR-1 Durable diagnosis projection - scope: migrate and backfill `PersistedLessonOpportunity.source_position_id` without guessing; persist the pre-move source position for new analyses; expose owner-authorized per-position diagnosis and game analysis-status projections; commits: "feat(api): project persisted diagnoses by source position"; verification: `uv run pytest tests/integration/test_schema_migrations.py tests/integration/test_analysis_persistence.py tests/integration/test_analysis_board_api.py`.
2. PR-2 Render evaluations and honest analysis state (on PR-1) - scope: per-position evaluation display, diagnosis markers, and distinct unanalysed/completed-without-findings/H-033-not-found states; offer an analyse action only for the unanalysed state; commits: "feat(web): show persisted engine evaluations on the analysis board", "feat(web): handle unavailable game analysis"; verification: `pnpm --dir apps/scan64-web test`.
...
VERIFICATION (must pass): `uv run pytest tests/integration/test_schema_migrations.py tests/integration/test_analysis_persistence.py tests/integration/test_analysis_board_api.py`, `pnpm --dir apps/scan64-web test`, and `scripts/check.sh` pass. Manual: open a locally analysed owned game and confirm evaluations and durable source-position markers render.
REVIEW:
Per PR:
- Scope matches its purpose; contracts match the reconciled plan; behavior is meaningfully tested.
...
M31 OWNERSHIP CONTRACT (H-002): the export/import/deletion completeness set includes `Game.owner_player_id` as player-derived ownership data in addition to the M31 analysis rows, M34's player-scoped `ProfileObservation`, M37's player-scoped `LessonAttempt`, and coach-student relation records.
OBJECTIVE: Export, import, and deletion cover every table holding player-derived data, including the evidence M31 begins writing, M34's profile observations, M37's lesson attempts, coach-student relations, and player-scoped transfer measurements; global `TransferPosition` references round-trip without being deleted. Acceptance: export-delete-import round-trips a player with analysed games, profile observations, lesson attempts, a transfer measurement and its global transfer-position reference, and a coach-student relation against an existing counterpart and leaves no orphan rows; after deletion every player-scoped application table has zero residual rows for that player, asserted per table rather than inferred from a success status; global `TransferPosition` remains unchanged; the required `DeletionAudit` tombstone is asserted independently and never rehydrated; adding a new player-scoped table without registering it fails the completeness test.
RELEASE TRAIN: target=v0.2.0 - "adaptive and operable"; included milestones=M38-M45; preparation trigger=all included milestones externally merged; required artifacts=pyproject.toml version bump, CHANGELOG.md release notes, uv build artifacts, clean-install scan64-cli smoke test; release verification=scripts/check.sh exits 0 on main after the final merge plus M44's clean-clone quickstart and M45's coach-mode interruption walkthrough; publication=explicit maintainer authorization required.
...
2. Inspect src/scan64/api/data_lifecycle.py:23-46,118-130,353-365, the DeletionAudit writer at :367-378, src/scan64/api/middleware.py, the full SQLModel table set, M31's merged evidence writes, M34's ProfileObservation schema, and M37's LessonAttempt schema with their CI evidence.
3. Revalidate objective, interfaces, dependencies, acceptance, verification, risks, release train, and dependent milestones listed in M42's Design reevaluation row (none). Confirm the set of player-scoped tables has not changed since the plan was written, `TransferPosition` remains global while `TransferMeasurement` is player-scoped, a shared coach-student relation restores only against an existing counterpart, and terminal deletion does not leave a cached idempotency response.
4. Append one ledger entry to .docs/DEVELOPMENT_PLAN_HISTORY.md: timestamp, milestone, decision, trigger, evidence, plan/prompt sections changed, downstream impact, implementation authorization.
5. If no material mismatch exists, report `DESIGN GO - PLAN REVISION: none`; this authorizes implementation.
...
2. PR-2 Export and import coverage (on PR-1) - scope: add Evidence, Position, EngineAnalysis, ProfileObservation, LessonAttempt, TransferMeasurement, StudySession, CoachStudentLink, and global TransferPosition references to the archive schema and import path; commits: "feat(privacy): export and import analysis and study data", "feat(privacy): preserve coach links in lifecycle archives"; verification: `uv run pytest tests/integration -k lifecycle_roundtrip`
3. PR-3 Deletion coverage and audit (on PR-2) - scope: delete the added player-derived tables but retain global TransferPosition rows, evict the authenticated player's idempotency records without caching the terminal deletion response, assert zero residual application rows per table, and update DeletionAudit records; commits: "feat(privacy): delete analysis and study data with the player", "feat(privacy): record deleted table coverage in the audit"; verification: `uv run pytest tests/integration/test_data_lifecycle_completeness.py -k deletion`

CONSTRAINTS: no scope leakage, minimal dependencies, repo style, no version/changelog updates. Do not define a hosted-mode retention policy; that is a documented GAP. Assert zero residual rows per player-scoped application table - never infer deletion success from a status code. The required DeletionAudit tombstone is not a rehydratable player row and is asserted separately. Include ProfileObservation, LessonAttempt, CoachStudentLink, and authenticated-principal idempotency cleanup in every applicable lifecycle path.
...
OBJECTIVE: The transfer-measurement lifecycle is usable through the API instead of only through tests, and lesson verification confirms the accepted move actually answers the objective. Acceptance: after a verified persisted-opportunity attempt moves an active skill to >= 0.80, exactly one production transfer lifecycle is assigned; its pre-test appears only in a later session as a required due item; completing it records a measurement and moves the player report without updating M37 profile or review state; a lesson whose accepted move is not engine- or tablebase-best fails verification; previously persisted lessons are re-verified on read, marked with a retained failure reason, excluded when invalid, and never deleted.
RELEASE TRAIN: target=v0.2.0 - "adaptive and operable"; included milestones=M38-M45; preparation trigger=all included milestones externally merged; required artifacts=pyproject.toml version bump, CHANGELOG.md release notes, uv build artifacts, clean-install scan64-cli smoke test; release verification=scripts/check.sh exits 0 on main after the final merge plus M44's clean-clone quickstart and M45's coach-mode interruption walkthrough; publication=explicit maintainer authorization required.

PRE-IMPLEMENTATION DESIGN GATE:
1. Read DEVELOPMENT_PLAN.md §6 M43, its §1 source-map rows, this prompt, and .docs/DEVELOPMENT_PLAN_HISTORY.md when present.
2. Inspect src/scan64/learning/evaluation/transfer_measurement.py:41-162, src/scan64/learning/exercises/transfer.py:13-141, src/scan64/learning/verification/verifier.py:26-40, the M43 transfer catalog, and the merged M37/M38 typed attempt and active-session paths with their CI evidence. Confirm the catalog can write every `TransferPosition` used by production reads, the M37 profile-recording boundary supplies only owned persisted opportunities, and transfer completion remains distinct from M37's ungraded opening_mission records and M30 controlled-study writers.
...
1. PR-1 Objective-correctness verification - scope: add the persisted verification state and strengthen `verify_lesson` so every accepted move equals an engine- or tablebase-proved objective move; reuse M31 `EngineAnalysis` before bounded Stockfish fallback; re-verify persisted lessons on read, mark failures with retained reasons, and exclude invalid lessons without deleting them; commits: "feat(learning): verify that an accepted move answers the objective", "feat(learning): re-verify persisted lessons on read"; verification: `uv run pytest tests/unit/test_lesson_verification.py`
2. PR-2 Transfer position seeding (on PR-1) - scope: add the M43 production transfer catalog with three distinct legal FEN-plus-UCI-solution records per seeded skill and idempotently seed `TransferPosition` rows with derived transfer metadata; commits: "feat(content): seed transfer positions from the catalog"; verification: `uv run pytest tests/integration/test_transfer_measurement.py -k seeding`
3. PR-3 Assignment and due selection (on PR-2) - scope: assign one production transfer lifecycle when a verified persisted-opportunity attempt leaves an active skill at mastery >= 0.80, using an opaque production lifecycle id rather than M30 enrolment; persist immutable target-move evidence; surface each due transfer measurement as a required `transfer_measurement` lesson in a later training session; commits: "feat(learning): assign transfer exercises at the mastery threshold", "feat(api): include due transfer items in a training session"; verification: `uv run pytest tests/integration/test_transfer_measurement.py -k assignment`
4. PR-4 Completion and report (on PR-3) - scope: accept the typed transfer attempt, record its measurement without M37 profile or review mutation, schedule post-tests, and expose an owner-authorized per-player report; commits: "feat(learning): record transfer measurements on completion", "feat(api): report transfer performance for a player"; verification: `uv run pytest tests/integration/test_transfer_measurement.py`

CONSTRAINTS: no scope leakage, minimal dependencies, repo style, no version/changelog updates. Do not touch the M30 controlled-study infrastructure. Never delete a lesson that fails re-verification - mark it. Do not assign transfer work from a retired taxonomy skill. Every session transfer lesson must be backed by seeded catalog data and immutable accepted target-move evidence.
