...
2. Inspect `AnalysisScreen.tsx`, M39's merged H-033 bearer-header behavior, M31's `/v1/games/{id}/positions` payload, the `PersistedLessonOpportunity` model, its Alembic history, and the analysis worker. Confirm every diagnosis has a durable source position; the owner-authorized API projects evaluations, diagnoses, and analysis state; and no read-time FEN recovery remains.
