...
Interactive play and batch analysis draw from separate, lifespan-bound Stockfish process pools (`SCAN64_ENGINE_POOL_INTERACTIVE_CONCURRENCY`, `SCAN64_ENGINE_POOL_BATCH_CONCURRENCY`; both default `2`), so a running batch analysis never queues a live move. Analysis jobs are admitted under a per-player daily quota (`SCAN64_ANALYSIS_DAILY_QUOTA`, default `50`); work beyond it is queued fair-share round-robin across players, never rejected or dropped.
